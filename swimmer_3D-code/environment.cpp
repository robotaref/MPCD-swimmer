...
//        M[5]->r=Zero;
        ...
        Ss.push_back(new spring(6,100000));
        Ss.push_back(new spring(6,100000));

        M[0]->AddSpring(Ss[0],-1);
        M[1]->AddSpring(Ss[0], 1);

        M[0]->AddSpring(Ss[1],-1);
        M[2]->AddSpring(Ss[1], 1);

        this->AS.push_back(new AngularSpring(pi,1000000));
        AS[0]->AssignMass(M[1],M[0],M[2]);
...
void environment::saveTrajectory(int time)
{
    ...
    if (counter%savingstep==0  )  // SAVING AND CALCULATION SECTION
    {
        ...
        if (showsolvent==1  )  // SAVING AND CALCULATION SECTION
        {
            ...
            trajectory <<HeavyNum<<endl;    // saving trajectories
            trajectory << " nodes  "<<endl;


            for(int j=0; j<HeavyNum;j++) // saving trajectory
            {
                trajectory << "s"<<  "\t"<<M[j]->r.x<< "\t" <<M[j]->r.y<< "\t" <<M[j]->r.z<<endl;

            }
            vector3d rcom(0,0,0);
            for(int kkk=0;kkk<HeavyNum;kkk++){
                rcom=rcom+M[kkk]->r;
            }
//            com<<"test";
            com<<i<<"\t"<<rcom.x/HeavyNum<<"\t"<<M[0]->v.x<<endl;
...
void environment::SwimmimgSenarioNG(int time)
{
    double dl=.005;
    if((time/500) %4 ==0){
        Ss[0]->stretch(-dl);
